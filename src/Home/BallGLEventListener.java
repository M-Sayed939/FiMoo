...
public class BallGLEventListener implements GLEventListener {
    ...
    public void init(GLAutoDrawable glAutoDrawable) {
        ...
        for (int x = (int) X_MIN; x < X_MAX; x += 10) {
...
//    updateDirection();
        for(int i = 0; i < MAX_BALLS; i++) {
            ...
            drawBall(gl, ballXPosition[i], ballYPosition[i], pointColorList.get(idx++));
            idx %= pointColorList.size();
        }
    }
     ...
     */
    private void drawBall(GL gl, double xBall, double yBall,PointColor pointColor) {
        System.out.println("----" + xBall + " " + yBall);
//    gl.glClear(GL.GL_COLOR_BUFFER_BIT);
        gl.glColor3f(pointColor.red, pointColor.blue, pointColor.green);
...
//    }
    ...
    public static void main(String[] args) {
        BallGLEventListener x = new BallGLEventListener();
        for(int i = 0; i < 10; i++) {
            System.out.println(x.randNum(0, 5));
        }
    }

}

//    private void drawWOW(GL gl, int r, Color color, int x, int y) {
